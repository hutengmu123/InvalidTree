...
from OCT_base import solve_oct_MILP, ModelTree, get_solution_from_CART, solve_modified_oct_MILP
...
class OptimalTreeClassifier:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        leaf_solution = {}
        node_sample = [2,3,4,5,6,7]
        leaf_sample = [8,9,10,11,12,13,14,15]
        leaf_dictionary = _get_dict_branch_t_leaf_m(max_depth=3)
        state = 0
        for value in leaf_sample:
            leaf_solution[value] = 0
        ...
        for element in node_sample:
            if model.d[element].value == 1:
                if model.b[element] == 0:
                    print("\x1b[31m\"invalid tree\"\x1b[0m")
                    state = 1
                    break
                else:
                    pass
            else:
                for value in leaf_dictionary[element]:
                    if leaf_solution[value] !=0:
                        print("\x1b[31m\"invalid tree\"\x1b[0m")
                        state = 1
                        break
...
class MOptimalTreeClassifier:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for element in node_sample:
            if model.d[element].value == 1:
                if model.b[element] == 0:
                    print("\x1b[31m\"modified invalid tree\"\x1b[0m")
                    state = 1
                    break
                else:
                    pass
